...
Этот раздел формализует визуальный язык проекта. Он служит руководством по использованию стандартных UI-компонентов для обеспечения единообразия, предсказуемости и высокого качества интерфейсов. Все описанные компоненты находятся в `components/DocUIComponents.tsx`.
