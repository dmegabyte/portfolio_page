...
import { 
    ...
    UserIcon, ArrowsRightLeftIcon, SparklesIcon, ArrowLongRightIcon, LightBulbIcon, WrenchScrewdriverIcon, PuzzlePieceIcon,
...
// A new local component to structure the Input/Output examples clearly.
const InputOutputExample: React.FC<{
  title: string;
  request: string;
  resultCode: string;
  resultTitle: string;
  requestType?: 'text' | 'code';
}> = ({ title, request, resultCode, resultTitle, requestType = 'text' }) => {
    return (
        <div className="my-8 not-prose">
            <h4 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-4">{title}</h4>
            <div className="bg-white/5 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
                {/* Input Section */}
                <div className="p-4 bg-slate-100 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <p className="font-semibold text-slate-800 dark:text-slate-200">Запрос:</p>
                            {requestType === 'text' ? (
                                <p className="text-slate-600 dark:text-slate-400 italic">"{request}"</p>
                            ) : (
                                <pre className="mt-2 text-xs bg-slate-200 dark:bg-slate-900 p-3 rounded-md custom-scrollbar overflow-x-auto text-slate-700 dark:text-slate-300">
                                    <code>{request}</code>
                                </pre>
                            )}
                        </div>
                ...
                <div className="p-3 text-center text-slate-400 dark:text-slate-600">
                    <ArrowLongDownIcon className="w-6 h-6 inline-block" />
                </div>
                ...
                <div className="px-1 sm:px-2 md:px-4 pb-1 sm:pb-2 md:pb-4">
                   <CodeBlockWithCopy 
                       title={resultTitle}
                       code={resultCode}
                       language={requestType === 'code' ? 'json' : undefined}
                   />
                </div>
            </div>
        </div>
...
const InterfaceGeneratorDocumentationPage: React.FC = () => {
    ...
    const miniDiagramModalDescriptions = {
        ...
        html: {
            ...
            step3: { title: "HTML+JS: 3. Добавление JS", content: <p>Внутри тега <code>&lt;script&gt;</code> генерируется `vanilla JS` код, который добавляет обработчики событий (`addEventListener`) и реализует описанную интерактивность (например, переключение классов для аккордеона).</p> },
        },
        json: {
    ...
    const CssModeGuide = () => (
        <div className="bg-gray-50 dark:bg-slate-900/50 rounded-xl p-6 border border-gray-200 dark:border-slate-700">
            ...
            {/* <CollapsibleSection title="Лучшие практики для запросов">
            ...
            <CollapsibleSection title="Пример ввода/вывода (CSS)">
                <InputOutputExample
                    title=""
                    request="Для ссылок в футере (footer a) убери подчеркивание, а при наведении делай его синим."
                    resultTitle="Результат (чистый CSS)"
                    resultCode={`footer a {
    ...
    const HtmlJsModeGuide = () => (
         <div className="bg-gray-50 dark:bg-slate-900/50 rounded-xl p-6 border border-gray-200 dark:border-slate-700">
            ...
            <CollapsibleSection title="Пример ввода/вывода (HTML + JS)">
                 <InputOutputExample
                    title="Пример: Модальное окно с анимацией"
                    request="Создай модальное окно с затемнением фона. Окно должно плавно появляться (scale + opacity) при клике на основную кнопку и закрываться при клике на 'Закрыть' или на фон."
                    resultTitle="Результат (HTML + JS + Tailwind)"
                    resultCode={`<!-- Кнопка вызова -->
<button id="openBtn" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-bold shadow-xl transition-all active:scale-95">
  Открыть модальное окно
</button>

<!-- Оверлей и само окно -->
<div id="modalOverlay" class="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
  <div id="modalContent" class="bg-white dark:bg-slate-900 w-full max-w-md p-8 rounded-[32px] shadow-2xl transform scale-90 transition-transform duration-300 border border-slate-200 dark:border-slate-800">
    <h3 class="text-2xl font-black text-slate-900 dark:text-white mb-4 tracking-tight">Подтверждение действия</h3>
    <p class="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed">
      Вы действительно хотите применить эти изменения? Это действие нельзя будет отменить.
    </p>
    <div class="flex gap-3">
      <button id="confirmBtn" class="flex-1 bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors">Да, применить</button>
      <button id="closeBtn" class="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white py-3 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">Отмена</button>
    </div>
  </div>
</div>
...
<script>
  ...
  "module": "Global",
  "template": "Header",
  "question": "Нужны компоненты для авторизации",
  "filename": "header.tpl"
}`}
                    resultTitle="Результат (чистый JSON)"
                    resultCode={`{
  "system_codes": [
    "$AUTH_LOGIN_FORM$",
    "$AUTH_REGISTER_LINK$",
  ...
  "descriptions": [
    "Стандартная форма для входа пользователя.",
    "Ссылка на страницу регистрации.",
    "Ссылка для восстановления забытого пароля."
  ]
}`}
                />
            </CollapsibleSection>
