...
export const TypographyTable: React.FC = () => {
    ...
    return (
        <div className="overflow-x-auto my-8 not-prose border border-slate-700 rounded-2xl bg-slate-900/50 backdrop-blur-xl shadow-2xl">
            <table className="w-full text-left border-collapse">
                <thead className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-widest border-b border-slate-700">
                    <tr>
                        <th className="p-6 font-black">Уровень</th>
                        <th className="p-6 font-black">Применение</th>
                        <th className="p-6 font-black">Классы Tailwind</th>
                        <th className="p-6 font-black">Превью</th>
                    </tr>
                </thead>
                <tbody className="text-slate-300 divide-y divide-slate-800">
                    {levels.map((item, index) => (
                        <tr key={index} className="hover:bg-indigo-500/5 transition-colors">
                            <td className="p-6 font-bold text-white whitespace-nowrap">{item.level}</td>
                            <td className="p-6 text-sm">{item.useCase}</td>
                            <td className="p-6 text-xs font-mono text-indigo-400">{item.tailwindClasses}</td>
                            <td className="p-6">
                                <span className={item.tailwindClasses}>{item.previewText}</span>
                            </td>
                        </tr>
                    ))}
                </tbody>
...
const highlightCode = (code: string, language?: 'json') => {
    ...
    const highlighted = code.trim().replace(
        ...
        (match, jsonToken, _stringContent, colon) => {
            if (jsonToken) {
                ...
                {data.map((row, rowIndex) => (
                    <tr key={rowIndex} className="border-b dark:border-slate-700 last:border-b-0">
                        {row.map((cell, cellIndex) => (
                            <td key={cellIndex} data-label={headers[cellIndex]} className="p-3 border-x border-gray-200 dark:border-slate-700">{cell}</td>
                        ))}
                    </tr>
                ))}
            </tbody>
...
// --- Portal for Tooltips and Modals ---
const Portal: React.FC<{ children: React.ReactNode; targetId: string }> = ({ children, targetId }) => {
...
export const TooltipTerm: React.FC<TooltipTermProps> = ({ children, definition }) => {
    ...
    const [arrowClasses, setArrowClasses] = useState('top-full');
    ...
    useLayoutEffect(() => {
        if (isVisible && tooltipRef.current && triggerRef.current) {
            ...
            const newTransform = shouldFlip ? 'translate(-50%, 0)' : 'translate(-50%, -100%)';
            const newArrowClasses = shouldFlip ? 'bottom-full rotate-180' : 'top-full';

            let newLeft = triggerRect.left + triggerRect.width / 2;
            const margin = 8;
            if (newLeft - tooltipWidth / 2 < margin) {
                newLeft = margin + tooltipWidth / 2;
            } else if (newLeft + tooltipWidth / 2 > viewportWidth - margin) {
                newLeft = viewportWidth - margin - tooltipWidth / 2;
            }
        ...
        <Portal targetId="modal-root">
            ...
            >
                ...
                >
                    ...
                    <div className="p-6 max-h-[70vh] overflow-y-auto text-base text-slate-700 dark:text-slate-300 custom-scrollbar">
                        {children}
                    </div>
                </div>
            </div>
...
// --- NEW, REBUILT Line Chart Components ---
...
const ChartTooltip: React.FC<{
    ...
    xKey: string;
    yKeys: YKeyConfig[];
}> = ({ dataPoint, anchorTop, anchorLeft, xKey, yKeys }) => {
    const tooltipRef = useRef<HTMLDivElement>(null);
    ...
    useLayoutEffect(() => {
        if (tooltipRef.current) {
            ...
            let newLeft = anchorLeft + spacing;
            if (newLeft + tooltipWidth > viewportWidth - margin) {
                newLeft = anchorLeft - tooltipWidth - spacing;
            }
            if (newLeft < margin) {
                newLeft = margin;
            }

            let newTop = anchorTop - tooltipHeight / 2;
            if (newTop < margin) {
                newTop = margin;
            }
    ...
    }, [anchorTop, anchorLeft, dataPoint]);
    ...
    return (
        <Portal targetId="tooltip-root">
            <div
                ...
                className="bg-slate-800/90 dark:bg-slate-900/90 backdrop-blur-sm border border-slate-700 rounded-lg shadow-2xl p-4 text-white font-sans"
            >
                <p className="font-bold text-lg mb-3 text-slate-100">{dataPoint[xKey]}</p>
                <div className="space-y-2">
                    {yKeys.map((yk) => (
                        <div key={yk.key} className="flex items-center justify-between text-base">
                            ...
                            <span className="font-bold text-slate-100">{dataPoint[yk.key]}</span>
                        </div>
                    ))}
                </div>
            </div>
...
export const LineChart: React.FC<LineChartProps> = ({ data, config, onPointClick }) => {
    ...
    const { chartWidth, chartHeight, xScale, yScaleLeft, yScaleRight, linePaths, yTicksLeft, yTicksRight } = useMemo(() => {
        ...
        const numTicks = 4;
        const yTicksLeft = Array.from({ length: numTicks + 1 }, (_, i) => ({ value: Math.round(yMaxLeft - (i * yMaxLeft / numTicks)), y: padding.top + i * (chartHeight / numTicks) }));
        const yTicksRight = Array.from({ length: numTicks + 1 }, (_, i) => ({ value: Math.round(yMaxRight - (i * yMaxRight / numTicks)), y: padding.top + i * (chartHeight / numTicks) }));
    ...
    const handlePointHover = useCallback((index: number | null) => {
        if (index !== null && data[index] && chartRef.current) {
            const chartRect = chartRef.current.getBoundingClientRect();
            const dataX_svg = xScale(index);
            const yValues_svg = yKeys.map(k => (k.axis === 'right' ? yScaleRight : yScaleLeft)(data[index][k.key]));
            const anchorTop = chartRect.top + (Math.min(...yValues_svg) + Math.max(...yValues_svg)) / 2;
            const anchorLeft = chartRect.left + dataX_svg;

            setTooltip({ index, dataPoint: data[index], anchorTop, anchorLeft });
        } else {
            setTooltip(null);
        }
    }, [data, xScale, yKeys, yScaleLeft, yScaleRight]);
    ...
    return (
        <div className="not-prose my-6 bg-[#1A202C] dark:bg-slate-900 rounded-xl p-6 border border-gray-700 dark:border-slate-700 overflow-hidden shadow-2xl">
            ...
            >
                ...
                <g onMouseLeave={() => handlePointHover(null)}>
                    {data.map((_, i) => {
                        ...
                        return (
                            <rect key={`hover-rect-${i}`} x={xScale(i) - sliceWidth / 2} y={padding.top} width={sliceWidth} height={chartHeight} fill="transparent"
                                onMouseEnter={() => handlePointHover(i)}
                                onClick={() => onPointClick?.(data[i])}
                                style={{ cursor: onPointClick ? 'pointer' : 'default' }}
                                aria-label={`Показать данные для периода ${data[i][xKey]}`}
                            />
                        );
                    })}
                </g>
            ...
            {tooltip && (
                <ChartTooltip dataPoint={tooltip.dataPoint} anchorTop={tooltip.anchorTop} anchorLeft={tooltip.anchorLeft} xKey={xKey} yKeys={yKeys} />
            )}
        </div>
    );
};
...
const HighlightComment: React.FC<{ text: string }> = ({ text }) => {
    // Split the text into the main part and the parenthesized example
    ...
    if (match && match[1]) {
        ...
        return (
            <p className="text-slate-300 text-sm mt-px">
                <strong className="font-semibold text-slate-200">{mainText}</strong>
                {exampleText && <span className="text-slate-400 ml-1">{exampleText}</span>}
            </p>
        );
    }

    // Fallback for texts that don't match the pattern
    return <p className="text-slate-300 text-sm mt-px">{text}</p>;
};
